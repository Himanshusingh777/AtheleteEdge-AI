...
def health_report():
    ...
    if request.method == 'POST':
        ...
        bmi = weight / (height ** 2)
        bmi_status = analyze_bmi(bmi)
        bp_status = analyze_bp(systolic, diastolic)
        sugar_status = analyze_sugar(fasting, post)
        chol_status = analyze_cholesterol(cholesterol)
        ...
        pdf.cell(200, 10, "Health Analysis:", ln=True)
        pdf.set_font("Arial", '', 12)
        pdf.cell(200, 10, f"BMI: {bmi:.2f} - {bmi_status}", ln=True)
        pdf.cell(200, 10, f"Blood Pressure: {systolic}/{diastolic} mmHg - {bp_status}", ln=True)
        pdf.cell(200, 10, f"Fasting Sugar: {fasting} mg/dL - {sugar_status}", ln=True)
        if post:
            pdf.cell(200, 10, f"Post-Meal Sugar: {post} mg/dL", ln=True)
        pdf.cell(200, 10, f"Cholesterol: {cholesterol} mg/dL - {chol_status}", ln=True)
